...
def do_jsp(instance):
    ...
    res, n_time = [], 10
    Utils.make_dir("./ResDe")
    Utils.make_dir("./ResDe/%s" % instance)
    for n_do in range(n_time):
        a.do_evolution()
        res.append([a.best[1], len(a.record[0]), a.best[0].problem.direction])
        Utils.save_code_to_txt("./ResDe/%s/Code%s" % (instance, n_do), a.best[0].code)
        Utils.save_record_to_csv("./ResDe/%s/Record%s" % (instance, n_do), a.record)
        print("Makespan: ", a.best[1])
    Utils.save_obj_to_csv("./ResDe/%s.csv" % instance, res)
