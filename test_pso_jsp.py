...
def do_jsp(instance):
    ...
    max_generation = 500
    c1, c2, w = 2, 2, 0.6
    a = PsoShopSchedule(pop_size, max_generation, c1, c2, w, my_problem, func, max_or_min=max_or_min)
    res, n_time = [], 10
    Utils.make_dir("./ResPso")
    Utils.make_dir("./ResPso/%s" % instance)
    for n_do in range(n_time):
        a.do_evolution()
        res.append([a.best[1], len(a.record[0]), a.best[0].problem.direction])
        Utils.save_code_to_txt("./ResPso/%s/Code%s" % (instance, n_do), a.best[0].code)
        Utils.save_record_to_csv("./ResPso/%s/Record%s" % (instance, n_do), a.record)
        print("Makespan: ", a.best[1])
    Utils.save_obj_to_csv("./ResPso/%s.csv" % instance, res)
