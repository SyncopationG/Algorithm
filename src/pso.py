...
class PsoShopSchedule(PsoNumericOptimization):
    def __init__(self, pop_size, max_generation, c1, c2, w, problem, func, max_or_min=0):
        PsoNumericOptimization.__init__(self, pop_size, max_generation, c1, c2, w, problem, func, max_or_min)
...
class PsoShopScheduleWorker(PsoShopSchedule):
    def __init__(self, pop_size, max_generation, c1, c2, w, problem, func, max_or_min=0):
        PsoShopSchedule.__init__(self, pop_size, max_generation, c1, c2, w, problem, func, max_or_min)
    ...
    def do_init(self, pop=None):
        ...
        for i in range(self.pop_size):
            if pop is None:
                ...
                code = pop[0][i].code
            info = self.problem.decode_worker_pso(self.func, code)
            self.pop[0].append(info)
            self.pop[1].append(info.obj)
            self.pop_best[0].append(info)
            self.pop_best[1].append(info.obj)
